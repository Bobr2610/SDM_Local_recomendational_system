...
Сначала соберите train_long (если ещё нет):
  python backend/scripts/data_prep/build_train_long.py

Затем:
  python backend/scripts/data_prep/profiles_from_train_long_quantiles.py
  python backend/scripts/data_prep/profiles_from_train_long_quantiles.py --apply

--apply  — перезаписать mobile/src/config/profiles.ts и
           frontend/src/features/profiles/ClientSelector.tsx числовыми полями
           (имена/тексты UI не трогает).
"""
...
def main() -> None:
    ...
    if not data.exists():
        raise SystemExit(
            f"No dataset at {data}. Run: python backend/scripts/data_prep/build_train_long.py"
        )
    ...
    if args.apply:
        ...
        frontend = ROOT / "frontend" / "src" / "features" / "profiles" / "ClientSelector.tsx"
